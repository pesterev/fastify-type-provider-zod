...
const createJsonSchemaTransform = ({ skipList }) => {
    return ({ schema, url }) => {
        ...
        for (const prop in zodSchemas) {
            ...
            if (zodSchema) {
                transformed[prop] = replaceAnyOfWithOneOf((0, zod_to_json_schema_1.zodToJsonSchema)(zodSchema, zodToJsonSchemaOptions));
            }
        }
        if (response) {
...
function replaceAnyOfWithOneOf(obj) {
    if (obj instanceof Array) {
        return obj.map((s) => replaceAnyOfWithOneOf(s));
    }
    if ('anyOf' in obj) {
